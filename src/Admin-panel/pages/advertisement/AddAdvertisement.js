...
function AddAdvertisement() {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = await ADD_ADS(FORM_DATA);
      ...
      <Card>
        {loading ? <><CulsightPageLoader /></> : <>
          <h2> <span style={{ cursor: "pointer" }}
            onClick={() => navigate("/advertisement")}><LeftOutlined /></span>Create Advertisement</h2>
          <Form form={form}
            layout="vertical"
            autoComplete="off"
            onFinish={onFinish}
            ...
            {/* Ads Type */}
            <Form.Item label="Ads Type" name="ads_type" initialValue="video">
