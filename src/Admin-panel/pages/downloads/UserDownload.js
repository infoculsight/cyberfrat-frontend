...
import { GET_DOWNLOAD_REPORT, LIST_DOWNLOAD_USER_REPORT } from "../../apis/apis";
...
function UserDownload() {
  ...
  const LIST_API = async (page = 1, perPage = page_size) => {
    ...
    const API_CALL = await LIST_DOWNLOAD_USER_REPORT(FORM_DATA);
  ...
  const columns = [
    {
      ...
      render: (text, record) => {
        try {
          let nameMatch = record.report_view.match(/'title':\s*'([^']+)'/);
