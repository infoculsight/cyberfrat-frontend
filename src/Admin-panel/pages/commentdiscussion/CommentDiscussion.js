...
import { DISSCUSSION_STATUS, LIST_APP_DISCUSSION } from "../../apis/apis";
...
function CommentDiscussion() {
    ...
    const change_status = async (id) => {
        ...
        FORM_DATA.append("type", "discussion_comment");
    ...
    const columns = [
        ...
        {
            title: "Replies",
            dataIndex: "count_discussion_reply",
            render: (text, record) => <span>{record.count_discussion_reply}</span>,
        },
        {
            title: "Status",
            key: "status",
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    {
                        record.count_discussion_reply > 0 ? (
                            <Tooltip title="Replies">
                                <Button type="primary" size="small" onClick={() => navigate("/discussion-replies/" + btoa(record.id))}><EyeFilled /></Button>
                            </Tooltip>
                        ) : (
                            <Tooltip title="Replies">
                                <Button type="primary" size="small" onClick={() => navigate("/discussion-replies/" + btoa(record.id))} disabled><EyeFilled /></Button>
                            </Tooltip>
                        )
                    }
    ...
    const LIST_API = async () => {
        ...
        const API_CALL = await LIST_APP_DISCUSSION(FORM_DATA);
